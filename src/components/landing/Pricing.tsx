        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-7xl w-full">
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`rounded-2xl sm:rounded-3xl p-6 sm:p-8 transition-all duration-300 cursor-pointer ${
                plan.featured
            ...
            >
              {plan.featured && (
                <motion.div
                  ...
                  className="absolute -top-3 sm:-top-4 left-1/2 -translate-x-1/2 bg-[#e0fe10] text-black px-4 sm:px-6 py-1 rounded-full text-xs font-bold"
                >
                  MOST POPULAR
                </motion.div>
              )}
              ...
              <div className="mb-6">
                <h3 className="text-gray-400 text-xs sm:text-sm uppercase tracking-wide mb-2">{plan.name}</h3>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-4xl sm:text-5xl md:text-6xl font-black text-white">{plan.price}</span>
                  {plan.price !== "Free" && plan.name !== "Calorie AI+" && (
                    <span className="text-gray-400 text-base sm:text-lg">/year</span>
                  )}
                  {plan.name === "Calorie AI+" && <span className="text-gray-400 text-base sm:text-lg">/month</span>}
                </div>
                <p className="text-gray-300 text-xs sm:text-sm">{plan.subtitle}</p>
              </div>

              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className={`w-full py-3 sm:py-4 rounded-full font-bold mb-6 sm:mb-8 transition-all text-sm sm:text-base ${
                  plan.featured ? "bg-[#e0fe10] text-black" : "bg-white text-black hover:bg-gray-200"
                }`}
                onClick={() => router.push("")}
              >
                Get Started
              ...
              <div className="space-y-3 sm:space-y-4">
                {plan.main && <div className="text-gray-400 text-xs sm:text-sm mb-2">{plan.main}</div>}
                {plan.features.map((feature, idx) => (
                  ...
                  >
                    <div
                      className={`rounded-full p-1 flex-shrink-0 ${plan.featured ? "bg-[#e0fe10]" : "bg-indigo-600"}`}
                    >
                      <Check className="w-3 h-3 text-black" />
                    </div>
                    <span
                      className={`text-xs sm:text-sm ${
                        feature.includes("No Ads") || feature.includes("Calorie AI")
